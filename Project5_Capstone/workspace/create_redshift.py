...
def create_clients(config):
    ...
    print('print aws region {}'.format(config['AWS']['REGION']))
...
def create_redshift_cluster(config, redshift, roleArn):
    ...
    print('Wait, we are checking the status of your {}'.format(config['DWH']['DWH_CLUSTER_IDENTIFIER']))
    ...
    while not is_create_redshift:
        ...
        print('Status cluster is: {}'.format(myClusterProps['ClusterStatus']))
...
def main():
    ...
    if (clusterProps):
        print(redshift_props(clusterProps))
